...
- Типичные продукты: гречка, рис, курица, говядина, хек/минтай, творог 5%, яйца, овсянка, бананы (но всегда проверяй реальный холодильник!)
...
async def ai_trainer(request: Request):
    ...
    # 2. Build messages — inject fridge status (empty or filled)
    if fridge_context:
        system_prompt += f"\n\nХОЛОДИЛЬНИК МАКСИМА (актуальное содержимое): {fridge_context}\nКОГДА СПРАШИВАЕТ ЧТО ПРИГОТОВИТЬ — используй ТОЛЬКО эти продукты. Не придумывай продукты которых нет в списке."
    else:
        system_prompt += "\n\nХОЛОДИЛЬНИК: пуст или не заполнен в приложении. Если спрашивает что приготовить — скажи что холодильник пуст и попроси сначала добавить продукты во вкладке 🧊 Холодильник."
